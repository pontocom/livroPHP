...
try {
	...
	$stmt =$dbo->prepare("UPDATE cliente SET nome='".$_REQUEST['nome']."', morada='".$_REQUEST['morada']."', telefone='".$_REQUEST['telefone']."' WHERE num=".$_REQUEST['num']);
	...
	{
	$dbo = new PDO("sqlite:bd_sqlite3_pdo.db"); 
	$stmt= $dbo->prepare("SELECT * FROM cliente WHERE num=".$_REQUEST['num']);
	$stmt->execute();      
	$row=$stmt->fetch();
?>
	<html>
